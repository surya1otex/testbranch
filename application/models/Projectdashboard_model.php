...
class Projectdashboard_model extends CI_Model {
...
//get user details	
 ...
 SELECT project_id,approve_status, 'Project Aggrement' as stage,'5' as stage_id FROM project_aggrement_stage WHERE approve_status='N') all_project ON all_project.project_id=id LEFT JOIN area_master ON area_master.id = project_conceptualisation_stage.project_area LEFT JOIN project_type_master ON project_type_master.id = project_conceptualisation_stage.project_type WHERE all_project.approve_status='N' and project_conceptualisation_stage.approver_id=".$user_id." ");
    ...
    // }
        ...
        public function get_project_data($project_id = Null)
        {
        $this->db->select('pd.*,area.name as area_name,dest.name as dest_name,sector.name as sector_name,typeP.project_type as ptype_name,group.name as groupName,orgapprover.firstname,orgapprover.lastname,t6.wing_name,t7.division_name');
        $this->db->from('project_conceptualisation_stage as pd');
        $this->db->where('pd.id', $project_id);
        $this->db->join('area_master as area', 'pd.location_id = area.id', 'LEFT');
        $this->db->join('destination_master as dest', 'pd.project_destination = dest.id', 'LEFT');
        ...
        $this->db->join('group_master as group', 'pd.project_group = group.id', 'LEFT');
        $this->db->join('organization_user_details as orgapprover', 'pd.approver_id = orgapprover.user_id', 'LEFT');
        $this -> db ->join('wing_master t6', 't6.id = pd.wing_id ', 'left');
        $this -> db ->join('division_master t7', 't7.id = pd.division_id ', 'left');
        $this->db->join('destination_master as destination', 'pd.project_destination = destination.id', 'LEFT');
        $query = $this->db->get();
        return $query->result_array();
        }
    ...
    {
        $this->db->select('ps.*');
        $this->db->from('tendering_pre_bid as ps');
        $this->db->where('ps.project_id', $project_id);
        $query = $this->db->get();
       	return $query->result_array();
    ...
    public function tender_publishing_project_data($project_id = Null)
    {
        $this->db->select('ps.*');
    ...
    public function communication_project_data($project_id = Null)
    {
        $this->db->select('ps.*');
        $this->db->from('project_communication as ps');
        $this->db->where('ps.project_id', $project_id);
        $query = $this->db->get();
       	return $query->result_array();
    ...
    public function issue_project_data($project_id)
    {
        
        $this -> db ->select('pc.project_name as projectname,uis.issues as issuename,uis.action_taken as actiontaken,uis.current_status as status,us.username as createdby');
        $this -> db ->from('project_conceptualisation_stage pc');
        $this -> db ->join('update_status_issues uis', 'pc.id = uis.project_id ', 'left');
        $this -> db ->join('user us', 'uis.entered_by = us.id ', 'left');

        $this -> db -> where('uis.project_id', $project_id);
...
GROUP by period");
    ...
    {
        $query = $this->db->query(" select all_project.project_id,project_name,project_code, all_project.approve_status,all_project.stage_id, all_project.stage,area_master.name as area_name,project_type_master.project_type from project_conceptualisation_stage left join (
 SELECT project_id,approve_status, 'Project Aggrement' as stage,'5' as stage_id FROM project_aggrement_stage WHERE approve_status='y') all_project ON all_project.project_id=id 
 ...
 LEFT JOIN project_type_master ON project_type_master.id = project_conceptualisation_stage.project_type WHERE all_project.approve_status='Y' and project_conceptualisation_stage.approver_id=".$user_id." ");
    ...
    public function get_all_activity_details($project_id, $work_item_id)
    {
        ...
        $this->db->select('pa.particulars as activity_name,project_financial_planning_detail.project_activity_id as activity_id,
        IFNULL(SUM(project_financial_planning_detail.target_amount), 0) as Planned_Value, IFNULL(SUM(project_financial_planning_detail.earned_amount), 0) as Earned_Value, IFNULL(SUM(project_financial_planning_detail.allotted_amount), 0) as Paid_Value');
        ...
        $this->db->join('project_pf_activities as pa', 'pa.id = project_financial_planning_detail.project_activity_id', 'LEFT');

        $this->db->group_by('project_financial_planning_detail.project_activity_id');
        $this->db->order_by('project_financial_planning_detail.id');
   ...
   public function unit_name_data($unit_id)
    ...
    //Project Issue
     ...
     function get_project_issue_data($project_id){
       
        // $this->db->select('pcs.project_name');
        // $this->db->from('project_conceptualisation_stage pcs');
    
        // $this -> db -> where('pcs.proj_rel_id', $project_id);
        // $query = $this->db->get();
        // $return = $query->result_array();
        // return $return[0];
        ...
        $this -> db -> where('pcs.id', $project_id);
        $query = $this->db->get();
        $return = $query->result_array();
        return $return[0];
    ...
    function get_project_issue_details($project_id){
        $this->db->select('pcs.project_name,uis.issues as issuename,uis.action_taken as actiontaken,uis.current_status as status,us.username as createdby,uis.timestamp as date');
        $this->db->from('project_conceptualisation_stage pcs');
    ...
    // function get_project_issue_pdf_details(){

    // $this->db->select('pc.project_name as projectname,usi.issues as issuename');
    // $this->db->from('update_status_issues usi');
    // $this -> db ->join('project_creation pc', 'usi.project_id = pc.id');
    // $query = $this->db->get();
    //     if($query -> num_rows() >= 1){ return $query->result(); }
    //     else{ return false; }
    // }
    ...
    public function get_work_item_activity_budget($project_work_item_id){
	  ...
	  function project_activity_amount_data($project_id, $project_activity_id){
        ...
        if($query->num_rows() > 0)
        {
	     ...
	     public function get_project_work_item_data($project_id)
    {
        $query = $this->db->query("
SELECT c.project_work_item_id, pd.work_item_description, IFNULL(SUM(target_amount), 0) as Planned_Value, IFNULL(SUM(earned_amount), 0) as Earned_Value, IFNULL(SUM(allotted_amount), 0) as Paid_Value
    FROM `project_financial_planning_detail` c
    left join work_item_master pd on c.project_work_item_id=pd.id 
	WHERE c.project_id=".$project_id."  GROUP BY c.project_work_item_id ORDER BY c.id   ");
       //echo  $this->db->last_query(); die();
        $result = $query->result_array();
        return $result;
    }
    ...
    public function get_financial_overview($project_id,$work_item_id,$activity_id){
		
		$conditionN = "";	
			  if($work_item_id > 0){  
        $conditionN = "AND c.project_work_item_id='$work_item_id'"; 
      }
      if($activity_id > 0){
        $conditionN = "AND c.project_activity_id='$activity_id'"; 
      }
...
SELECT c.id,c.month_date,DATE_FORMAT(c.month_date, '%Y-%m') as period, IFNULL(SUM(target_amount), 0) as Planned_Value, IFNULL(SUM(earned_amount), 0) as Earned_Value, IFNULL(SUM(allotted_amount), 0) as Paid_Value
	...
	WHERE c.project_id=".$project_id." $conditionN  GROUP BY DATE_FORMAT(c.month_date, '%Y-%m') ORDER BY c.month_date
");
    ...
    {
        $query = $this->db->query("
SELECT c.project_activity_id, pd.particulars, IFNULL(SUM(c.target_amount), 0) as Planned_Value, IFNULL(SUM(c.earned_amount), 0) as Earned_Value, IFNULL(SUM(c.allotted_amount), 0) as Paid_Value
    FROM `project_financial_planning_detail` c
    left join project_pf_activities pd on c.project_activity_id=pd.id 
	WHERE c.project_id=".$project_id."  GROUP BY c.project_activity_id ORDER BY c.id   ");
      //echo  $this->db->last_query(); die();
        $result = $query->result_array();
        return $result;
    }
	  ...
	  public function get_project_finishdate($project_id, $work_item_id,$activity_id)
    {
	  ...
	  function get_project_tendering_steps_data($project_id){

        $query = $this->db->query(" select sum(prebid) prebid_status, sum(techeva) techeva_status, sum(fineva) fineva_status, sum(negoti) negoti_status, sum(issue) issue_status, sum(aggre) aggre_status, astatus from 
                (SELECT count(*)
                 as prebid, 0 as techeva, 0 as fineva, 0 as negoti, 0 as issue, 0 as aggre, approval_status as astatus  FROM `tendering_pre_bid` WHERE project_id=".$project_id."
                UNION
                SELECT 0 as prebid, count(*)
                 as techeva, 0 as fineva, 0 as negoti, 0 as issue, 0 as aggre, approval_status as astatus FROM `tendering_technical_evalution` WHERE project_id=".$project_id."
                UNION
                SELECT 0 as prebid, 0 as techeva, count(*)
                 as fineva, 0 as negoti, 0 as issue, 0 as aggre, approval_status as astatus FROM `tendering_financial_evalution` WHERE project_id=".$project_id."
                UNION
                SELECT 0 as prebid, 0 as techeva, 0 as fineva, count(*)
                 as negoti, 0 as issue, 0 as aggre, approval_status as astatus  FROM `tendering_negotiation` WHERE project_id=".$project_id."
                UNION
                SELECT 0 as prebid, 0 as techeva, 0 as fineva, 0 as negoti, count(*)
                 as issue, 0 as aggre, approval_status as astatus FROM `tendering_issue_of_loa` WHERE project_id=".$project_id."
                UNION
                SELECT 0 as prebid, 0 as techeva, 0 as fineva, 0 as negoti, 0 as issue, count(*)
                 as aggre, 0 as astatus FROM `project_aggrement_stage` WHERE project_id=".$project_id.") as summary");
      // echo  $this->db->last_query(); die();
    ...
    function get_project_pre_construction_overview_percent($project_id){
        $query =  $this->db->query("Select pldp.`progress_%` AS Land_alination,pla.`progress_%` AS Direct_purchase,plac.`progress_%` AS Land_acquistion,fl.`progress_%` AS forest_land,ue.`progress_%` AS utility_shifting_electrical,usp.`progress_%` AS utility_shifting_ph,usrw.`progress_%` AS utility_shifting_RWSS,tc.`progress_%` AS tree_cutting,ev.`progress_%` AS encrochment_eviction FROM pre_construction_settings AS pcs
left JOIN pre_construction_activities_govt_land_alienation AS pla ON pcs.project_id = pla.project_id 
left JOIN pre_construction_activities_pvt_land_direct_purchase AS pldp ON pcs.project_id = pldp.project_id
 left JOIN pre_construction_activities_pvt_land_acquistion AS plac ON pcs.project_id = plac.project_id 
left JOIN pre_construction_activities_forest_land AS fl ON pcs.project_id = fl.project_id 
left JOIN pre_construction_activities_utility_shifting_electrical AS ue ON pcs.project_id = ue.project_id 
left JOIN pre_construction_activities_utility_shifting_ph AS usp ON  pcs.project_id = usp.project_id 
left JOIN pre_construction_activities_utility_shifting_rwss AS usrw ON  pcs.project_id = usrw.project_id 
left JOIN pre_construction_activities_tree_cutting AS tc ON pcs.project_id = tc.project_id 
left JOIN pre_construction_activities_encroachment_eviction AS ev ON pcs.project_id = ev.project_id WHERE pcs.project_id='".$project_id."'");
    ...
    function get_project_pre_construction_cost_overview($project_id){
        $query =  $this->db->query("Select pldp.progress_fund_utilised AS Land_alination,pla.progress_fund_utilised AS Direct_purchase,plac.progress_fund_utilised AS Land_acquistion,fl.progress_fund_utilised AS forest_land,ue.progress_fund_utilised AS utility_shifting_electrical,usp.progress_fund_utilised AS utility_shifting_ph,usrw.progress_fund_utilised AS utility_shifting_RWSS,tc.progress_fund_utilised AS tree_cutting,ev.progress_fund_utilised AS encrochment_eviction FROM pre_construction_settings AS pcs 
LEFT JOIN pre_construction_activities_govt_land_alienation AS pla ON pcs.project_id = pla.project_id
 LEFT JOIN pre_construction_activities_pvt_land_direct_purchase AS pldp ON pcs.project_id = pldp.project_id 
LEFT JOIN pre_construction_activities_pvt_land_acquistion AS plac ON pcs.project_id = plac.project_id 
LEFT JOIN pre_construction_activities_forest_land AS fl ON pcs.project_id = fl.project_id 
...
LEFT JOIN pre_construction_activities_encroachment_eviction AS ev ON pcs.project_id = ev.project_id WHERE pcs.project_id='".$project_id."'");
	...
	/**** TENDERING ADDED BY ESTPL TEAM ************/
	
	function get_doc_files($communication_id,$project_id) {
		$query = $this->db->query("select project_communication_document.document_name,project_communication_document.communication_file FROM
        project_communication_document WHERE project_communication_document.communication_id='".$communication_id."' AND project_communication_document.project_id='".$project_id."'");
      ...
      // $this->db->select('*');
      // $this->db->where('project_communication_document.project_id', $project_id);
      // $query = $this->db->get('project_communication_document');
      // return $query->result_array();

	}
     function fetch_tendering_technical_evalution_single_data($tbl, $fid, $did)
      {
        $this -> db -> select('*');
        $this -> db -> from($tbl);
        $this -> db -> where($fid, $did);
    ...
    function get_tendering_technical_evalution_bidder_data($project_id){
     ...
     function fetch_tendering_financial_evalution_single_data($tbl, $fid, $did)
      {
        $this -> db -> select('*');
        $this -> db -> from($tbl);
        $this -> db -> where($fid, $did);
    ...
    function get_tendering_financial_evalution_bidder_data($project_id){
      ...
      $query = $this->db->query("select tfeb.bidder_ref_no as biddername,tfeb.bid_value as bidvalue,tfeb.successful_bidder as status,tfeb.final_score as score from tendering_financial_evalution AS tfe  JOIN tendering_financial_evalution_bidder_details AS tfeb ON tfe.project_id = tfeb.project_id WHERE tfe.project_id='".$project_id."'");
        ...
        // $query = $this->db->query("select ttebd.bidder_ref_no as biddername,tfeb.bid_value as bidvalue,tfeb.successful_bidder as status,tfeb.final_score as score from tendering_financial_evalution AS tfe  JOIN tendering_financial_evalution_bidder_details AS tfeb ON tfe.project_id = tfeb.project_id JOIN tendering_technical_evalution_bidder_details AS ttebd ON ttebd.id = tfeb.bidder_ref_no WHERE tfe.project_id='".$project_id."'");
     ...
     function fetch_tendering_negotiation_single_data($tbl, $fid, $did)
      {
        $this -> db -> select('*');
        $this -> db -> from($tbl);
        $this -> db -> where($fid, $did);
      ...
      function get_tendering_negotiation_bidder_data($project_id){
      ...
      $query = $this->db->query("select tfeb.bidder_ref_no as biddername,tnb.negotiation_meeting_date as meetingdate,tnb.negotiation_bid_value as bidvalue,tnb.successful_bidder_response as document,tnb.negotiation_status as status from tendering_negotiation AS tn JOIN tendering_negotiation_bidder_details AS tnb ON tn.project_id = tnb.project_id JOIN tendering_financial_evalution_bidder_details AS tfeb ON tnb.bidder_id = tfeb.id WHERE tn.project_id='".$project_id."'");
        ...
        // $query = $this->db->query("select ttebd.bidder_ref_no as biddername,tnb.negotiation_meeting_date as meetingdate,tnb.negotiation_bid_value as bidvalue,tnb.successful_bidder_response as document,tnb.negotiation_status as status from tendering_negotiation AS tn JOIN tendering_negotiation_bidder_details AS tnb ON tn.project_id = tnb.project_id JOIN tendering_financial_evalution_bidder_details AS tfeb ON tnb.bidder_id = tfeb.id JOIN tendering_technical_evalution_bidder_details AS ttebd On ttebd.id=tnb.bidder_id WHERE tn.project_id='".$project_id."'");
     ...
     function fetch_tendering_issue_of_loa_single_data($tbl, $fid, $did)
      {
        $this -> db -> select('*');
        $this -> db -> from($tbl);
        $this -> db -> where($fid, $did);
     ...
     function fetch_tendering_pre_bid_single_data($tbl, $fid, $did)
      {
        $this -> db -> select('*');
        $this -> db -> from($tbl);
        $this -> db -> where($fid, $did);
      ...
      function get_tendering_pre_bid_bidder_data($project_id){

      $query = $this->db->query("select pbd.bidder_name,pbd.first_name,pbd.middle_name,pbd.last_name,pbd.mobile_no,pbd.email_id FROM tendering_pre_bid_bidder_details AS pbd WHERE pbd.project_id='".$project_id."'");
     ...
     function get_tendering_pre_bid_bidder_data_document($project_id){
      ...
      $this->db->select('*');
      $this->db->where('project_communication.project_id', $project_id);
   ...
   public function get_individual_data($project_id) {
       

        $query =  $this->db->query("select tendering_pre_bid.project_id AS `Pre-Bid`,tendering_technical_evalution.project_id AS `Technical Evaluation`,tendering_financial_evalution.project_id AS `Financial Evaluation`,tendering_negotiation.project_id AS Negotiation,tendering_issue_of_loa.project_id AS Issue_of_loa FROM tendering_pre_bid
          LEFT JOIN tendering_technical_evalution ON tendering_pre_bid.project_id = tendering_technical_evalution.project_id
          LEFT JOIN tendering_financial_evalution ON tendering_pre_bid.project_id = tendering_financial_evalution.project_id
          LEFT JOIN tendering_negotiation ON tendering_pre_bid.project_id = tendering_negotiation.project_id
          LEFT JOIN tendering_issue_of_loa ON tendering_pre_bid.project_id = tendering_issue_of_loa.project_id WHERE
          tendering_pre_bid.project_id='".$project_id."'");


          $datesql = $this->db->query("select tendering_pre_bid.approval_date AS prebiddate,tendering_technical_evalution.approval_date AS tecndate,tendering_financial_evalution.approval_date AS Financialdate,tendering_negotiation.approval_date AS Negotiationdate,tendering_issue_of_loa.approval_date AS loadate FROM tendering_pre_bid
          LEFT JOIN tendering_technical_evalution ON tendering_pre_bid.project_id = tendering_technical_evalution.project_id
          LEFT JOIN tendering_financial_evalution ON tendering_pre_bid.project_id = tendering_financial_evalution.project_id
          LEFT JOIN tendering_negotiation ON tendering_pre_bid.project_id = tendering_negotiation.project_id
          LEFT JOIN tendering_issue_of_loa ON tendering_pre_bid.project_id = tendering_issue_of_loa.project_id WHERE
          ...
          if($datesql->num_rows() > 0)
          {

            $dateres = $datesql->row_array();

                $date = [];

                foreach($dateres as $key=>$val) {
            ...
            $result = $query->row_array();
             ...
             foreach($result as $key=>$val) {
                 ...
                 if($val != '') {
                       ...
                       $project_status = array("name" => $key, "value" => $val, "description" => $date[$index],"label" => $date[$index]);

                       array_push($progress, $project_status);
  ...
  public function get_project_total_data($project_id) {
	...
	public function get_issues_details($project_id) {


        $this -> db ->select('pcs.project_name as projectname,uis.issues as issuename,uis.id,uis.action_taken as actiontaken,uis.current_status as status,uis.timestamp as date,us.username as createdby');
        $this -> db ->from('project_conceptualisation_stage pcs');
        $this -> db ->join('update_status_issues uis', 'pcs.id = uis.project_id ', 'left');
        $this -> db ->join('user us', 'uis.entered_by = us.id ', 'left');
    ...
    // SELECT pc.project_name as projectname,usi.issues as issuename,usi.action_taken as actiontaken,usi.current_status as status,us.username as createdby FROM project_creation pc LEFT JOIN update_status_issues usi on pc.id = usi.project_id LEFT JOIN user us ON usi.entered_by=us.id  where usi.project_id=1 AND usi.current_status = 'Y'
    }

    public function get_issues_details_open($project_id) {


        $this -> db ->select('pcs.project_name as projectname,uis.issues as issuename,uis.id,uis.action_taken as actiontaken,uis.current_status as status,us.username as createdby');
        $this -> db ->from('project_conceptualisation_stage pcs');
        $this -> db ->join('update_status_issues uis', 'pcs.id = uis.project_id ', 'left');
        $this -> db ->join('user us', 'uis.entered_by = us.id ', 'left');
    ...
    $query = $this->db->get();
    $result = $query->result();
    $mdlissue = $result[0]->issues;

    return $mdlissue;

    //return $this->db->get()->result()->row('issues');
