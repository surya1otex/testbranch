...
public function get_source_fund()
   ...
   /*checking num rows against 2 field*/
    ...
    function get_overview_project_details($project_id){
        $this->db->select('t1.project_name,t3.project_type as category_name,t4.name as location,t5.name as scheme,t6.progress_% as tree_%,t7.progress_% as ph_%,t8.progress_% as rwss_%,t9.progress_% as eviction_%,t10.progress_% as govt_land_%,t11.progress_% as pvt_land_direct_purchase_%,t12.progress_% as land_acquistion_%,t13.progress_% as forest_land_%,t14.progress_% as shifting_electrical_%');
        ...
        $this->db->join('pre_construction_activities_govt_land_alienation as t10', 't10.project_id = t1.id', 'LEFT');
        $this->db->join('pre_construction_activities_pvt_land_direct_purchase as t11', 't11.project_id = t1.id', 'LEFT');
        $this->db->join('pre_construction_activities_pvt_land_acquistion as t12', 't12.project_id = t1.id', 'LEFT');
        $this->db->join('pre_construction_activities_forest_land as t13', 't13.project_id = t1.id', 'LEFT');
    ...
    if($project_wing_id > 0){
      ...
      if($project_status == 'Pending'){
        ...
        $this->db->join('wing_master as t10', 't10.id = t1.wing_id', 'LEFT');
...
//      function get_delayed_project_data($project_group_id,$project_category_id,$project_area_id,$project_wing_id,$project_division_id){
...
//         $query = $this->db->query("select all_project.*,project_conceptualisation_stage.project_name,project_conceptualisation_stage.project_code, project_conceptualisation_stage.estimate_total_cost,district_master.district_name as area_name,project_type_master.project_type as category_name,draft_mode,division_master.division_name,group_master.name as scheme from project_conceptualisation_stage 
//             left join 
//             (
//              Select *  from
...
//              LEFT JOIN division_master ON division_master.id = project_conceptualisation_stage.division_id 
//              LEFT JOIN group_master ON group_master.id = project_conceptualisation_stage.project_group 
...
public function get_delayed_project_data($project_group_id,$project_category_id,$project_area_id,$project_wing_id,$project_division_id){
    ...
    function get_project_ammount_data($project_id){
        $query = $this->db->query("SELECT IFNULL(SUM(total_activity_budget_amount), 0) as Planned_Value, IFNULL(SUM(total_activity_earned_amount), 0) as Earned_Value, IFNULL(SUM(total_activity_allotted_amount), 0) as Paid_Value,project_id FROM `project_financial_planning_main` WHERE project_id='".$project_id."';");
        return $query->result_array();
    }

    function get_project_target_end_date($project_id){
        $query = $this->db->query("SELECT MAX(month_date) as target_end_date FROM project_financial_planning_detail WHERE earned_amount > 0 AND project_id='".$project_id."';");
        return $query->result_array();
    }
    
    //issue

     function get_issue_summary_report_data()
      {
        $this->db->select('pcs.project_name as project_name,uis.issues as issuename,uis.action_taken as actiontaken,uis.current_status as status,us.username as createdby,uis.timestamp as date');
        $this->db->from('update_status_issues uis');
        
         $this -> db ->join('project_conceptualisation_stage pcs', 'uis.project_id = pcs.id', 'left');
...
{
        $this -> db -> select('tendering_pre_bid.approval_date,project_conceptualisation_stage.project_name as tendername,tendering_pre_bid_bidder_details.bidder_name,country_master.country_name as country,tendering_pre_bid_bidder_details.first_name,tendering_pre_bid_bidder_details.last_name,tendering_pre_bid_bidder_details.mobile_no,tendering_pre_bid_bidder_details.email_id');



        $this->db->join('project_tender_stage', 'tendering_pre_bid.project_id = project_tender_stage.project_id');

        $this->db->join('project_conceptualisation_stage', 'project_tender_stage.project_id = project_conceptualisation_stage.id');
...
function get_technical_data_summary_report_data(){
        ...
        $this -> db -> select('project_conceptualisation_stage.project_name as tendername,tendering_technical_evalution.approval_date as tendering_date,tendering_technical_evalution_bidder_details.bidder_ref_no as bidder_tech_name,tendering_technical_evalution_bidder_details.technical_status as bidder_tech_status');
...
function get_issue_of_loa_data_summary_report_data(){
        ...
        $this -> db -> select('project_conceptualisation_stage.project_name as tendername,tendering_issue_of_loa.successful_bidder_ref_no,tendering_issue_of_loa.loa_issue_date,tendering_issue_of_loa.negotiation_bid_value');
   ...
   function fetch_single_tendering_pre_bid_report_data($tbl, $fid, $did)
      {
        $this -> db -> select('*');
        $this -> db -> from($tbl);
        $this -> db -> where($fid, $did);
