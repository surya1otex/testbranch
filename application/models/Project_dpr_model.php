...
class Project_dpr_model extends CI_Model
{
    ...
    function __construct()
    {
	    ...
	    public function checkProjectExits( $project_id,$table_name = 'project_dpr_stage'){
        $this->db->select('*');
   ...
   function getSpecificdata($table,$field,$get_id,$specifc_field){
     ...
     public function get_amount_brakup($project_id)
    {
	   ...
	   public function getProjectDprDetails( $project_id ){

        $this->db->select('*');
        $this->db->from('project_dpr_stage');
        $this->db->where('project_id', $project_id);
        $query = $this->db->get();
        $return = $query->result_array();
        return $return[0];
	    ...
	    public function updateProjectDPRDetails($data,$project_id )
    {
        $this->db->where('project_id', $project_id);
		//echo "<pre>";
		//print_r($data);die();
		//echo  $this->db->last_query(); 
        return $this->db->update('project_dpr_stage', $data);
    }
	
	    public function addProjectDPR($data)
    {

        $insert = $this->db->insert('project_dpr_stage', $data);
        if ($insert) {
