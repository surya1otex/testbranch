...
class Project extends MY_Controller
{
    ...
    public function project_conceptualisation(){
        ...
        $data['categorys'] = $this->Project_model->getProjectCategory();
        $data['circles'] = $this->Project_model->getProjectCircle();
        $data['divisions'] = $this->Project_model->getProjectDivision();
       ...
       // }
        ...
        if($draft == 'D'){
          ...
          $this->form_validation->set_rules('expected_approval_date', 'Expected Approval Date'); 
      }
	  	else{
        ...
        $this->form_validation->set_rules('estimate_total_cost', 'Estimated project cost', 'required');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $db_data['estimate_total_cost'] = $this->checkEmptyValue(str_replace(',','',$_REQUEST['estimate_total_cost']));
//===================================//
             $db_data['project_type'] = $this->checkEmptyValue($_REQUEST['project_category']);

             $db_data['wing_id'] = $this->checkEmptyValue($_REQUEST['project_circle']);
             $db_data['division_id'] = $this->checkEmptyValue($_REQUEST['project_division']);
...
function getdivision_list(){
          ...
          if($circle_id!=''){
            $data['all_divisions'] = $this->Project_model->fetch_divisions($circle_id);
            echo  json_encode($data);
          }else{
 ...
 function getdivision_data($wing_id,$division_id){
        ...
        if($wing_id != 0 || $circle_id != ''){
            ...
            $all_divisions = $this->Project_model->fetch_divisions($wing_id);
