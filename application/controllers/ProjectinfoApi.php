			  ...
			  // exit;
               foreach($project_work_item_detail as $pro_dtl){
				...
				//$project_type= $this->project_type($pro_dtl['project_type']);
				  ...
				  array_push($project_work_item, array("work_item_id" => $pro_dtl['work_item_id'], "stage_name" => $project_item[0]['work_item_description']));
				}
  ...
  public function update_financial_progress() {
     ...
     $method = $_SERVER['REQUEST_METHOD'];
     	if($method != 'POST'){
            ...
            if (!is_dir('uploads/earnedimages/')) {
                      mkdir('./uploads/earnedimages', 0777, TRUE);

                  }

            $target_dir = "uploads/earnedimages/";
       ...
       // update individual boxes end
          ...
          if($chk_exist > 0){
            ...
            //for add
               ...
               $chkprjimgexst = $this->Monitoring_model->earnedimagesexist($_REQUEST['project_id']);

              if(!empty($chkprjimgexst)) {
                 $this->Monitoring_model->removelist($_REQUEST['project_id']);
                }
      //individual box updates
      foreach($_REQUEST['financialPlanningDetailId'] as $key => $val){

// image upload section
     $earndimgsloopcnt = $_FILES['earnedimages']['name'][$key];

        // start of bulk iamges uploads
      for($j = 0; $j < count($earndimgsloopcnt); $j++) {


              $f_name = preg_replace('/\s+/', '_', $_FILES['earnedimages']['name'][$key][$j]);
               ...
               // Check file size
              if ($_FILES["earnedimages"]["size"][$key][$j] > 50000000) {
                 $sizeOk = 0;
                }
               else {
                    $sizeOk = 1;
       ...
       if ($sizeOk == 0 || $uploadOk == 0) {
             ...
             echo json_encode(['status' => 'Image size exceeded limit']);
           } 

       else {

           move_uploaded_file($_FILES["earnedimages"]["tmp_name"][$key][$j], $target_file);

            $earnedimage['project_id'] =  $_REQUEST['project_id'];
            $earnedimage['project_work_item_id'] = $_REQUEST['work_item_id'];
            $earnedimage['project_activity_id'] =  $_REQUEST['activity_id'];
            $earnedimage['project_financial_planning_id'] = $main_financial_id;
            $earnedimage['month_name'] = $_REQUEST['month'][$key];
           // $earnedimage['image'] = $_FILES['earnedimages']['name'][$key][$j];
            $earnedimage['image'] = $fileName;
            $earnedimage['latitude'] = $_REQUEST['latitude'][$key][$j];
            $earnedimage['longitude'] = $_REQUEST['longitude'][$key][$j];
            ...
            $img_detail = $this->Monitoring_model->check_table_data_exist_or_not_condition('earned_value_images',$where_detail_check);
             ...
             // if($img_detail > 0) {
             //  	 	//update images
             //       $this->Monitoring_model->updateimageCollection('earned_value_images',$earnedimage,$where_detail_check);
             ...
             //  	 }
              ...
              $this->Monitoring_model->insertimageCollection($earnedimage,'earned_value_images');
              	 // } 
 ...
 // end of multidimension images loop 
